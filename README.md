...
成员和贡献者都在自己的 DSH 中安装插件，通过团队页面操作：
...
以下供 DSH 维护者配置服务连接，或供希望自己维护团队后端、数据和网络入口的人使用。已连接可用团队服务的 DSH，可以直接通过页面创建团队。

### 连接已有团队服务

如果团队页面提示尚未配置，请由 DSH 维护者配置团队服务地址。在现有 `dsh-codex-shared-pool` 插件条目的 `config` 下设置：
