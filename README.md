...
成员和贡献者都要在自己的 DSH 中安装插件。使用已经提供的团队服务时，不需要自己部署服务器，直接在页面创建或加入团队即可：

1. 管理员打开 **设置 → Codex 订阅池 → 团队**，点击团队名称菜单中的 **创建团队**，填写团队名称和昵称即可成为所有者。
2. 管理员在团队面板创建邀请码并分享给成员。
3. 贡献者在团队页面选择要共享的本机账号，通过浏览器完成单独的共享授权，设置每周共享上限。
...
普通用户不需要执行这一节。只有希望自己维护团队后端、数据和网络入口时，管理员才需要在自有服务器运行下面的 Docker Compose。它不是创建团队的前置步骤。
