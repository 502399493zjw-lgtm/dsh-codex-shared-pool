...
成员和贡献者都在自己的 DSH 中安装插件。全新安装 `0.1.6` 默认提供云端团队入口，无需配置服务地址，直接通过团队页面操作：
...
**0.1.6：完善团队邀请、共享账号授权和请求记录，继续适配官方 DSH `0.1.2-rc.1` / Cordis `4.0.2`。**

- **共享账号请求数据**：团队成员可查看他人共享账号最近 1 天统计和近 7 天的最新 10 条共享请求；修改共享设置仍仅限贡献者。
- **邀请更清楚**：成员可查看已有可用邀请码，创建、撤销仍仅限所有者；加入入口先预览邀请信息再确认。
- **授权与连接修复**：首次共享采用浏览器实际授权的账号；退出、移除或解散的团队会从已保存连接中清理。
- **页面更简洁**：隐藏团队服务器地址，简化本机账号标题和优先级展示；保留默认云端入口及已有显式连接配置。

成员查看邀请码和共享账号请求数据需要团队服务端同步升级。npm/GitHub 发布不会自动更新正在运行的云端服务；旧服务端未提供共享请求数据时，页面显示暂不可用。已显式指向 `localhost` 或 `127.0.0.1` 的旧配置仍需由维护者修正。详见 [0.1.6 发布说明](docs/releases/0.1.6.md) 和 [团队连接与恢复](docs/team-connection.md)。
...
团队成员可查看共享账号最近 1 天的共享请求次数和 API 等价值估算，并通过“查看 7 天”查看最近 7 天内的最新 10 条共享请求（消耗人、模型、状态、时间及 Token 数）。仅当前团队可见的共享账号开放这项数据；账号自身的个人请求不计入，共享设置仍仅贡献者可修改。此功能需要团队服务端与客户端同步升级；旧服务端未提供数据时显示“暂不可用”。
...
当前安装版本为 `0.1.6`，适配官方 DSH `0.1.2-rc.1` / Cordis `4.0.2`。npm `0.1.3` 仍适用于旧基线 DSH `0.1.0-rc.8`。
...
团队成员可在 **团队设置 → 邀请码** 查看有效邀请码的说明和到期时间，点击 **查看** 后可复制。创建和撤销邀请码仍仅限所有者；成员查看功能需要本机插件与团队服务端均更新到包含此修复的版本。
...
以下供 DSH 维护者配置其他服务连接，或供希望自己维护团队后端、数据和网络入口的人使用。全新安装 `0.1.6` 的普通成员无需执行这些配置，默认即可使用云端创建、加入入口。
