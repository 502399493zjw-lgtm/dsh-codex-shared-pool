...
function providerMessage(value: unknown): string | undefined {
  ...
  return raw === undefined ? undefined : safeExternalErrorMessage(raw)
}
...
function abortable<T>(operation: Promise<T>, signal: AbortSignal): Promise<T> {
  if (signal.aborted) return Promise.reject(signal.reason instanceof Error ? signal.reason : new Error('OpenAI Codex image request aborted'))
  return new Promise<T>((resolve, reject) => {
    const onAbort = (): void => {
      reject(signal.reason instanceof Error ? signal.reason : new Error('OpenAI Codex image request aborted'))
    }
...
export class OpenAICodexImageClient {
   ...
   *
   * @param prompt - Complete generation or edit instruction.
   * @param images - Ordered data-URL reference images.
   * @param signal - Cancellation signal for authentication and HTTP work.
  ...
  async generate(
    ...
    const data = payload['data'].map((item: unknown) => item)
    const first = data[0]
 ...
 *
 * @param ctx - Tool and filesystem service context.
 * @param credentials - Refreshable OAuth credential source.
 * @param policy - Cross-provider image-tool policy.
 * @returns Executable image generation tool definition.
 */
