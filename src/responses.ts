...
/** Apply Codex Fast only where the official client catalog exposes it. */
function withFastServiceTier(payload: JsonRecord, model: Model<Api>, enabled: boolean): JsonRecord {
  return enabled && supportsCodexFastMode(model.id)
...
function markerOutput(text: string): readonly unknown[] | undefined {
  ...
  return parsed.map((item: unknown) => item)
}

/**
 * Replace a framed Harness checkpoint with the native items it durably carries.
 *
 * @param input - Provider input items reconstructed from the session log.
 * @returns Input items with valid native compaction markers expanded.
 */
...
function failedStream(model: Model<Api>, error: unknown, signal?: AbortSignal): AssistantMessageEventStream {
  ...
  const failure: AssistantMessage = {
    ...
    errorMessage: safeExternalErrorMessage(error),
    timestamp: Date.now(),
  }
  queueMicrotask(() => {
...
async function waitForRetry(delay: number, signal?: AbortSignal): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    ...
    if (signal.aborted) {
      reject(signal.reason instanceof Error ? signal.reason : new Error('OpenAI Codex request aborted'))
      return
    }
    const onAbort = (): void => {
...
async function compactResponse(response: Response, retained: readonly unknown[]): Promise<CompactResponse> {
  ...
  if (!state.completed) throw new Error('OpenAI Codex compact stream ended before response.completed')
...
export class OpenAICodexResponseRuntime {
   ...
   *
   * @param sessionId - Optional durable session identifier.
   * @returns Idempotent scope release callback.
   */
   ...
   *
   * @param provider - Base pi-ai provider implementation.
   * @returns Provider decorated with Codex request behavior.
   */
  ...
  private standardStream(
    ...
    return provider.streamSimple(model, context, {
      ...
      onPayload: async (payload, payloadModel) => {
        ...
        const customized = options?.onPayload === undefined
          ? transformed
          : await options.onPayload(transformed, payloadModel) ?? transformed
        if (!isRecord(customized)) throw new Error('OpenAI Codex payload customization returned a non-object')
        return withFastServiceTier(customized, payloadModel, this.preferences().useFastMode)
      },
    })
  }
  ...
  private async requestNativeCompaction(
    ...
    if (options?.onPayload !== undefined) {
      body = await options.onPayload(body, model)
    }
    if (!isRecord(body)) throw new Error('OpenAI Codex generated a non-object compact Responses payload')
    body = withFastServiceTier(body, model, this.preferences().useFastMode)
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      const detail = safeExternalErrorMessage(await response.text())
