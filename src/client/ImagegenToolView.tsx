...
function resultParts(block: ToolCallViewProps['block']): {
  ...
  const result = block as unknown as { content: readonly unknown[]; isError: boolean }
  let image: ImageAttachmentRef | undefined
  let text = ''
  for (const value of result.content) {
    if (typeof value !== 'object' || value === null || Array.isArray(value)) continue
    const item = value as Record<string, unknown>
    if (item['type'] === 'image' && image === undefined) image = imageAttachment(item['attachment'])
...
export function ImagegenToolView({ block, toolName, openFile, inspect, loadImage, t }: ImagegenToolViewProps) {
  ...
  return (
    <div style={rootStyle} data-tool="imagegen">
      ...
      {savedPath === undefined ? null : (
        <div style={savedRowStyle}>
          <span>{t('imageSavedAs')}</span>
          <button type="button" style={savedButtonStyle} title={savedPath} onClick={() => { openFile(savedPath) }}>{savedPath}</button>
        </div>
      )}
