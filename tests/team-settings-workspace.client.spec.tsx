...
it('portals member actions outside the clipped workspace and restores trigger focus', async () => {
  render(<TeamSettings t={translate} embedded />)
  const settings = await openTeamSettings('members')
  const trigger = within(settings).getByRole('button', { name: '管理 Mia' })
  fireEvent.click(trigger)
...
it('places return navigation before the join heading and returns without changing the active team', async () => {
  render(<TeamSettings t={translate} embedded />)
  fireEvent.click(await screen.findByRole('button', { name: '周末造物局' }))
  fireEvent.click(screen.getByRole('menuitem', { name: zh.joinTeamAction }))
  const back = screen.getByRole('button', { name: zh.returnToTeam })
  const heading = screen.getByRole('heading', { name: zh.joinOtherTeam })
  ...
  fireEvent.click(back)
  expect(await screen.findByRole('button', { name: '周末造物局' })).toBeDefined()
  expect(screen.queryByRole('heading', { name: zh.joinOtherTeam })).toBeNull()
  expect(managementApi.disconnect).not.toHaveBeenCalled()
  expect(managementApi.switchConnection).not.toHaveBeenCalled()
  expect(managementApi.join).not.toHaveBeenCalled()
})


it('keeps connection details available on demand and explains the invitation field accessibly', async () => {
  managementApi.status.mockResolvedValue({ enabled: true, keyConfigured: false, keyWritable: true, serverOrigin: 'https://team.example.test' })
  render(<TeamSettings t={translate} embedded />)
  const token = await screen.findByLabelText(zh.inviteToken)
  const descriptionId = token.getAttribute('aria-describedby')
  expect(descriptionId).toBeTruthy()
  expect(document.getElementById(descriptionId!)?.textContent).toBe('粘贴好友发来的完整邀请，先查看团队信息，再确认加入。')
  const details = screen.getByText('连接与安全').closest('details')!
  expect(details).not.toBeNull()
  expect(details.open).toBe(false)
  expect(within(details).getByText(zh.notConnectedHint)).toBeDefined()
  fireEvent.click(within(details).getByText('连接与安全'))
  expect(details.open).toBe(true)
  expect(within(details).getByText('团队服务地址：https://team.example.test')).toBeDefined()
  fireEvent.change(token, { target: { value: 'invalid' } })
  expect(token.getAttribute('aria-invalid')).toBe('true')
  expect(document.getElementById(descriptionId!)?.textContent).toBe(zh.inviteTokenInvalid)
  expect((screen.getByRole('button', { name: zh.previewInvitation }) as HTMLButtonElement).disabled).toBe(true)
  fireEvent.change(token, { target: { value: `dsh_invite_${'a'.repeat(32)}` } })
  expect(token.hasAttribute('aria-invalid')).toBe(false)
  fireEvent.click(screen.getByRole('button', { name: zh.previewInvitation }))
  expect(await screen.findByText(zh.invitationVerified)).toBeDefined()
  expect(managementApi.join).not.toHaveBeenCalled()
})
