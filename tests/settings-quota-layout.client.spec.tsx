...
describe('local account quota presentation', () => {
  ...
  it.each([en, zh])('leads account choices with recognizable names, followed by priority and readable status', async locale => {
    ...
    const rank = within(work).getByTitle(t('priorityPosition', { rank: 1 }))
    expect(within(rank).getByText('#1').getAttribute('aria-hidden')).toBe('true')
    expect(within(rank).getByText(t('priorityPosition', { rank: 1 }))).toBeDefined()
    expect(rank.getAttribute('title')).toBe(t('priorityPosition', { rank: 1 }))
    expect(await within(work).findByRole('img', { name: locale.accountConnected })).toBeDefined()
    expect(screen.queryByText(locale.accountConnected)).toBeNull()
    expect(within(work).getByText(locale.profileInUse)).toBeDefined()
    const detail = screen.getByRole('region', { name: 'Work Pro' })
    const heading = within(detail).getByRole('heading', { name: 'Work Pro' })
    const action = within(detail).getByRole('button', { name: locale.setPriorityProfile })
    expect(heading.closest('.dsh-codex-detail-heading')?.contains(action)).toBe(true)
    expect(within(detail).getByRole('status').getAttribute('aria-label')).toBe(locale.accountConnected)
    ...
    expect(within(screen.getByRole('region', { name: fallback })).getByRole('status').textContent).toBe(locale.accountConnectionUnavailable)
  })
})
