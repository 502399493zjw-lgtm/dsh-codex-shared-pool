...
function runtimeHarness(initialReuse: boolean, initialFast = false) {
  ...
  return {
    ...
    model,
    call,
    setFast(value: boolean): void { fast = value },
...
describe('OpenAICodexResponseRuntime transport policy', () => {
  ...
  it('adds and removes Codex priority service tier through the live Fast setting', async () => {
    const harness = runtimeHarness(false, true)
    harness.call('session-fast')
    expect(await harness.streamOptions[0]?.onPayload?.({ store: false, input: [] }, harness.model))
      .toEqual({ store: false, input: [], service_tier: 'priority' })

    harness.setFast(false)
    harness.call('session-default-speed')
    expect(await harness.streamOptions[1]?.onPayload?.({ store: false, input: [] }, harness.model))
      .toEqual({ store: false, input: [] })
  })
