...
function availableQuota(): Response {
...
describe('OpenAI Codex compaction request', () => {
  it('keeps stateless reasoning and paired tool history without sending a standard output cap', async () => {
    ...
    vi.stubGlobal('fetch', async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url === OpenAICodex.OPENAI_CODEX_USAGE_URL) return availableQuota()
      request = { url, init }
  ...
  it('uses native compact output as a durable checkpoint and restores its response items', async () => {
    ...
    vi.stubGlobal('fetch', async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (request.url === OpenAICodex.OPENAI_CODEX_USAGE_URL) return availableQuota()
    ...
    const responseRequests = requests.filter(request => request.url === OPENAI_CODEX_RESPONSES_URL)
    expect(responseRequests).toHaveLength(2)
    const compactBody = requestJson(responseRequests[0]!.init)
    const compactHeaders = new Headers(responseRequests[0]!.init.headers)
    ...
    const continuedBody = requestJson(responseRequests[1]!.init)
  ...
  it('falls back to Harness compaction when the native endpoint is unavailable', async () => {
    ...
    const responseRequests = requests.filter(request => request.url === OPENAI_CODEX_RESPONSES_URL)
    expect(responseRequests).toHaveLength(2)
    expect(requestJson(responseRequests[1]!.init).input).toEqual([
      { role: 'user', content: [{ type: 'input_text', text: 'keep this request' }] },
