...
After the required tests and verification checks pass, the agent should by default create focused commits, push the task branch, and open a draft pull request. Do not include unrelated user-owned changes in the commits. Do not create a remote repository, rewrite history, publish packages, or deploy to shared environments unless the user explicitly authorizes that action or the 3181 exception below applies.

Every agent-created pull request must receive an independent subagent review before it is marked ready or merged. The review must focus on the changes introduced by the pull request relative to its target branch: the changed behavior and implementation, tests added or modified for the change, affected interfaces and security boundaries, and compatibility risks. Read adjacent code only where necessary to understand the impact of the diff; this review is not a general audit of the entire repository. Pre-existing issues outside the changed scope should not block the pull request unless the change introduces, exposes, or materially worsens them.

Fix blocking findings in the same pull request and request another change-scoped subagent review. Once the review has no blocking findings and the required checks pass, mark the pull request ready and merge it without waiting for separate user confirmation. Never merge with failing required checks or unresolved blocking review findings.
...
Report the user-visible result, files changed, commands actually run, failures or unverified risks, and exact Git state. When a pull request exists, also report its target branch, verification status, subagent review conclusion, fixes made in response to review, and merge status.
